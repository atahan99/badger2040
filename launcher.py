# Badger 2040 launcher, patched from the stock BadgerOS one.
#
# The only change is the ORDER list below - see "local change". Everything else
# is upstream (pimoroni/badger2040, badger_os/launcher.py). If you update the
# firmware, diff this against the new stock launcher.py before keeping it.
import gc
import os
import time
import math
...
# --- local change: explicit page order -----------------------------------
# Stock BadgerOS shows apps in os.listdir() order, which littlefs returns
# alphabetically - so the useful apps scatter across pages by first letter.
# ORDER puts them where they belong; anything not listed (a new app you drop
# in) still shows up, appended alphabetically at the end.
ORDER = [
    "badge", "qrgen", "image",      # page 1 - the ones you actually open
    "dino", "life", "snake",        # page 2 - games
    "p2048", "dice", "count",       # page 3 - games and tallies
    "timer", "cheats", "list",      # page 4 - desk tools
    "batt", "clock", "help",        # page 5 - status
    "ebook", "info", "fonts",       # page 6 - reading and system
]

installed = [x[:-3] for x in os.listdir(APP_DIR) if x.endswith(".py")]
examples = [a for a in ORDER if a in installed]
examples += sorted(a for a in installed if a not in ORDER)
# --- end local change -----------------------------------------------------
